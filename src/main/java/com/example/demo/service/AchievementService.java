...
public class AchievementService {
	...
	public List<Achievement> checkAchievement(int userId, HttpSession session) {
		...
		Map<String, Supplier<Boolean>> map = getAchievementCheckMethods(cart, userId);
	...
	public Map<String, Supplier<Boolean>> getAchievementCheckMethods(List<CartItem> cart, int userId) {
		...
		map.put("isSapporoStampCompleted", () -> isSapporoStampCompleted(userId));
		map.put("isHakodateStampCompleted", () -> isHakodateStampCompleted(userId));
		map.put("isFuranoStampCompleted", () -> isFuranoStampCompleted(userId));
		map.put("isHokkaidoRoundTrip", () -> isHokkaidoRoundTrip(userId));
		map.put("isBestComboJingisukanBeer", () -> isBestComboJingisukanBeer(cart));
		map.put("isSeafoodComboSet", () -> isSeafoodComboSet(cart));
	...
	public boolean isBestComboJingisukanBeer(List<CartItem> cart) {
		if (cart != null) {
			...
			for (CartItem cartItem : cart) {
				if (cartItem.getName().contains("ジンギスカン")) {
					hasJingisukan = true;
				}
				if (cartItem.getName().contains("ビール")) {
					hasBeer = true;
				}
			}
	...
	public boolean isSeafoodComboSet(List<CartItem> cart) {
		if (cart != null) {
			...
			for (CartItem cartItem : cart) {
				if (cartItem.getName().contains("カニ") || cartItem.getName().contains("蟹")
						|| cartItem.getName().contains("ガニ") || cartItem.getName().contains("かに")
						|| cartItem.getName().contains("がに")) {
					hasKani = true;
				}
				if (cartItem.getName().contains("イクラ") || cartItem.getName().contains("いくら")) {
					hasIkura = true;
				}
				if (cartItem.getName().contains("ホタテ") || cartItem.getName().contains("ほたて")
						|| cartItem.getName().contains("帆立")) {
					hasHotate = true;
				}
			}
